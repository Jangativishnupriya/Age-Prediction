...
option = st.radio("Choose Input Mode:", ("📸 Use Webcam (Smile to Capture)", "🖼️ Upload Image", "🖼️ Gallery"))
...
if option == "📸 Use Webcam (Smile to Capture)":
    ...
    while True:
        ...
        if smiling:
            ...
            # Save captured image in gallery
            filename = os.path.join(GALLERY_DIR, f"{uuid.uuid4().hex}_{label}.jpg")
            Image.fromarray(img_rgb).save(filename)
            st.info(f"📸 Image saved to gallery: {filename}")
            break
...
# ====== UPLOAD MODE ======
    ...
    if uploaded_file is not None:
        ...
        filename = os.path.join(GALLERY_DIR, f"{uuid.uuid4().hex}_{label}.jpg")
        Image.fromarray(img_rgb).save(filename)
        st.info(f"🖼️ Image saved to gallery: {filename}")

# ====== GALLERY MODE ======
else:
    st.subheader("🖼️ Saved Gallery")
    images = os.listdir(GALLERY_DIR)

    if len(images) == 0:
        st.warning("No images saved yet! Capture or upload one first.")
    else:
        cols = st.columns(3)
